...
export interface TienduCartApi {
	...
	open(callback?: (payload: TienduCartOpenPayload) => void): Promise<HTMLIFrameElement>
	addProductVariant(
...
export type SubscriberAddResult = { success: true } | { success: false; errorCode: string }
...
export type QueryParamValue = string | number | boolean | null | undefined | Array<string | number>
export type QueryParams = Record<string, QueryParamValue>

export interface TienduClient {
	products: {
		list(options?: QueryParams): Promise<{ data: ProductListing[]; pagination?: { total: number; page: number; size: number } }>
		get(productId: number): Promise<ProductData>
		getRelated(productId: number): Promise<ProductListing[]>
	}
	reviews: {
		list(options?: QueryParams): Promise<{ data: unknown[]; pagination?: { total: number; page: number; size: number } }>
	}
	categories: {
